...
class `MSnExp`. Below, we first extract the full path to the
`MS3TMT11.mzML` file from the `msdata` package before reading it in.
...
## Using the `readMSnSet2` function
...
associated to a protein group, as defined by the `Proteins` feature
variable and aggregate them using a preferred operation.


```{r featsfig2, fig.cap = "Examples of aggregation from PSMs, peptides to proteins using median aggregation.", echo = FALSE}
...
We obtain `r nrow(cptac_prot)` proteins. Note how the processing steps are recorded.

```{r mds1}
plotMDS(exprs(cptac_prot), col = as.numeric(as.factor(cptac_prot$condition)))
```
...
plotMDS(exprs(cptac_rob), col = as.numeric(as.factor(cptac_rob$condition)))
```
</details>
...
In this section, the median-summarised protein data will be used. As
an exercice, you are advised to use the best of the median- or
robust-summerised data and compare your results with those shown
below.
...
to `FALSE`, this functionality will be removed from the package in the
next release.
...
A longer version with additional links and material of this lab is
available
