...
for quantification data via the `MSnSet` class (see below). Both store
...
> identification data with the TMT_Erwinia
...
> files from `msdata`. Retain only MS level data; this can be done
...
does data integrity checks throughout the data processing.
...
function as shown above, the data is read with `readMzIdData`, as
shown above, and is cleaned up:

- only PSMs matching the regular (non-decoy) database are retained;
- PSMs or rank greater than 1 are discarded;
- only proteotypic peptides are kept.

This behaviour can be replicates with the
`filterIdentificationDataFrame` function.
