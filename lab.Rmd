...
Note that if the file is already available, it is not downloaded a
second time.
...
```{r hdpeaks, fig.caption="Manual extraction and plotting of an MS spectrum"}
hd[1000, ]
head(peaks(ms, 1000))
...
Below, we setup and run the search.
...
allocated to the java virtual machine to 1GB. In general, there is no
need to specify this argument, unless you experience an error
