...
digested with a protease such as trypson. In mass shotgun proteomics,
the analytes assayed in the mass spectrimeter are peptides.


![A chromatogram ](./Figures/chromatogram.png)


![Schematics of a mass spectrometer.](./Figures/SchematicMS2.png)
...
metabolomics. More often, such *experiment packages* distribute
processed data; an example of such is the `r BiocStyle::Biocexptpkg("pRolocdata")` 
package, that offers quantitative proteomics data.

For this workflow, the the
*[PXD000001](https://bitbucket.org/lgatto/pxd000001)* package
distributes the files from the PXD000001 experiment. This is not an
official package, but is use full to pre-install the relevant data
files for local access. To install the package, we use the
`install_bitbucket` function from the `r CRANpkg("devtools")` package:

```{r bitbucket, eval=FALSE}
library("devtools")
install_bitbucket("lgatto/PXD000001")
```

Files can then be copied to the working directory using the `px1get`
function. See the manual page for details.
