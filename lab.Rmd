...
to `MSnSet` data. We will also demonstrate some specific functions of the
...
```{r knnopt, cache = TRUE}
p <- knnOptimisation(hyperLOPIT2015, time = 10)
p
```
...
A wide range of classification and clustering algorithms are available
in `r Biocpkg("MLInterfaces")`, as described in the `?MLearn`
