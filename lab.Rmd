...
a group effort by different developers in the community. This
[post](https://lgatto.github.io/msnbase-contribs/) illustrates the
various contributions to `r Biocpkg("MSnbase")`; `r Biocpkg("mzR")`
has benefited by a similar wide range of successful
...
MS2 spectra. The filename, `r basename(fileName(ms))`, is not very
...
- `r Biocpkg("MSstats")`: A set of tools for statistical relative
  protein significance analysis in DDA, SRM and Data independent
  acquisition (DIA) experiments. Data stored in `data.frame` or
  `MSnSet` objects can be used as input.
